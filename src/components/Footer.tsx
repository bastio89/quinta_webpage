...
export function Footer() {
  return (
    <footer className="bg-ink-950 py-16 text-mist-400">
      <div className="container-quinta">
        <div className="grid gap-12 lg:grid-cols-[1.4fr_2fr]">
          ...
          <div className="grid grid-cols-2 gap-8 sm:grid-cols-3">
            {COLUMNS.map((col) => (
              <div key={col.title}>
                <p className="text-xs font-semibold uppercase tracking-wide text-mist-200">{col.title}</p>
                <ul className="mt-4 space-y-2.5">
                  {col.links.map((link) => (
                    <li key={link.label}>
                      <Link href={link.href} className="text-sm text-mist-500 hover:text-mist-100">
                        {link.label}
                      </Link>
                    </li>
                  ))}
        ...
        <div className="mt-14 flex flex-col items-center justify-between gap-4 border-t border-white/10 pt-8 text-xs text-mist-600 sm:flex-row">
