...
export function Architecture() {
  return (
    <section id="architektur" className="bg-stone-50 py-24 sm:py-32">
      <div className="container-quinta">
        <div className="mx-auto max-w-2xl text-center">
          <p className="kicker text-azul-600">Architektur</p>
        ...
        <div className="mx-auto mt-16 max-w-5xl">
          ...
          <div className="flex flex-col items-stretch gap-3 lg:flex-row lg:items-center">
            <div className="card-surface flex-1 px-5 py-5 text-center">
              <p className="font-mono text-xs text-ink-500">Client · SDK · CLI</p>
              <p className="mt-1 text-sm font-medium text-ink-900">POST /v1/chat/completions + API-Key</p>
            </div>
            <ArrowRight className="mx-auto hidden h-5 w-5 shrink-0 text-ink-400 lg:block" />
            <ArrowDown className="mx-auto h-5 w-5 shrink-0 text-ink-400 lg:hidden" />

            {FLOW.map((step, i) => (
              <div key={step.name} className="contents">
                <div className="flex-1 rounded-xl border border-azul-200 bg-azul-50 px-5 py-5 text-center shadow-card">
                  <step.icon className="mx-auto h-6 w-6 text-azul-700" strokeWidth={1.75} />
                  <p className="mt-2 text-sm font-semibold text-ink-950">{step.name}</p>
                  <p className="font-mono text-[11px] text-azul-700">{step.role}</p>
                </div>
                {i < FLOW.length - 1 ? (
                  <>
                    <ArrowRight className="mx-auto hidden h-5 w-5 shrink-0 text-ink-400 lg:block" />
            ...
            <ArrowRight className="mx-auto hidden h-5 w-5 shrink-0 text-ink-400 lg:block" />
            <ArrowDown className="mx-auto h-5 w-5 shrink-0 text-ink-400 lg:hidden" />
            <div className="flex-1 rounded-xl border border-copper-300 bg-copper-50 px-5 py-5 text-center shadow-card">
              <p className="text-sm font-semibold text-ink-950">KI-Modell</p>
              <p className="font-mono text-[11px] text-copper-700">Lokale Inferenz-Runtime</p>
            </div>
          </div>
          ...
          <div className="mt-6 grid gap-4 sm:grid-cols-3">
            {SUPPORT.map((item) => (
              <div key={item.name} className="card-surface p-6">
                <item.icon className="h-6 w-6 text-azul-700" strokeWidth={1.75} />
                <p className="mt-3 text-sm font-semibold text-ink-950">
                  {item.name} <span className="font-mono text-xs font-normal text-ink-500">· {item.pkg}</span>
                </p>
                <p className="font-mono text-[11px] uppercase tracking-wide text-azul-700">{item.role}</p>
